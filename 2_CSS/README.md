...
- Flex 컨테이너에 'display: flex;' 적용시키면서 시작
...
- flex-direciton과 flex-wrap을 한까번에 지정할 수 있는 속성
...
```css
.container {
  align-content: stretch/ flex-start / flex-end/ center/ space-around/
    space-evenly;
}
```
...
- flex 아이템의 기본 크기 설정 (flex-direction)이 row일 때는 너비, column일 때는 높이)
...
- flex-grow와 쌍을 이루는 속성으로, flex-basis 값보다 작아질 수 있는지 결정하는 속성
